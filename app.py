...
def db_conn_funct():
    global mycon, cur
    mycon = pymysql.connect(host='localhost', user='root', password='', db='pbl', charset='utf8mb4')
    cur = mycon.cursor()
...
global bf, sf, lf, df, af, abf, cf, cnt_temp_chutiya
bf, sf, lf, df, af, abf, cf ,cnt_temp_chutiya= 0, 0, 0, 0, 0, 0, 0,0
...
# functions :
def billing():
    global bf, sf, lf, df, af, abf, cf
    cnt_temp_chutiya = 1
    sf, lf, df, af, abf, cf = 0, 0, 0, 0, 0, 0
...
def add_products():
    global bf, sf, lf, df, af, abf, cf
    bf,sf, lf, df, af, abf, cf = 0, 0, 0, 0, 0, 0,0
    hide_all_frames()
...
stats_menu.add_command(label="Statistics", command=bill)
...
# BILLING
# top window - custmer details
# global song
# song = input("D:\mmcoe\pbl\main-python-code\scanner_sound.wav")

def billing_app_page():
    # print("in billing app")
    global cnt_temp_chutiya,song,cap,c_bill_txt,cphn_txt,addproduct_e,cname_txt,discount_txt,total_bill_txt, addquantity_e ,capp,cap,mstr_df_prd ,cam1, cam2,sac4_e,sac3_e,sac2_e,sac1_e,cur, mycon,bill_id_new,bc_list_bill, pid_list_bill, quant_list_bill, name_list_bill, mrp_list_bill, total_list_bill,bc_list_bill_all
    db_conn_funct()
    quant_list_bill = pd.DataFrame()
    total_list_bill = pd.DataFrame()

    mstr_df_prd = pd.DataFrame({'pname':[], 'mrp':[], 'barcode':[],'pid':[],'quantity':[] , 'total':[],'profit':[], 'total_profit':[]})
    ...
    F1 = LabelFrame(billing_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)
    fc1 = LabelFrame(F1, fg="Black", bg="#badc57")
    fc1.grid(row=0, column=0, padx=40, pady=3)

    fc2 = LabelFrame(F1, fg="Black", bg="#badc57")
    fc2.grid(row=0, column=1, padx=37, pady=3)

    fc3 = LabelFrame(F1, fg="Black", bg="#badc57")
    fc3.grid(row=0, column=2, padx=40, pady=3)

    cname_lbl = Label(fc1, text="Customer Name:", bg="#badc57", font=('times new roman', 15, 'bold'))
    cname_lbl.grid(row=0, column=0, padx=10)
    cname_txt = Entry(fc1, width=15, font='arial 15')
    cname_txt.grid(row=0, column=1, padx=10)

    cphn_lbl = Label(fc2, text="Email:", bg="#badc57", font=('times new roman', 15, 'bold'))
    cphn_lbl.grid(row=0, column=2, padx=10)
    cphn_txt = Entry(fc2, width=23, font='arial 15')
    cphn_txt.grid(row=0, column=3, padx=10)

    c_bill_lbl = Label(fc3, text="Bill Number:", bg="#badc57", font=('times new roman', 15, 'bold'))
    c_bill_lbl.grid(row=0, column=4, padx=10)
    c_bill_txt = Entry(fc3, width=15,font=('times new roman', 15, 'bold'),state='disable')
    ...
    F2 = LabelFrame(billing_frame, fg="Black", bg="#badc57")
    F2.place(x=0, y=43, relwidth=1)

    barcode = LabelFrame(F2, fg="Black", bg="#badc57")
    barcode.grid(row=0, column=0, padx=5, pady=3)

    items = LabelFrame(F2, fg="Black", bg="#badc57", height=500)
    items.grid(row=0, column=1, padx=20, pady=3)

    # labels
    barcode_lbl = Label(barcode, text="Barcode", bg="#badc57", font=('times new roman', 15, 'bold'), width=50, height=20)
...
#============================= bill id gen ======================================
    ...
    import string
    import random
    bill_id_new = ''.join(random.choices(string.ascii_letters + string.digits, k=8))
...
#===================================================================
    ...
    manual_mode = LabelFrame(items, fg="Black", bg="#badc57")
    manual_mode.grid(row=0, column=0, padx=0, pady=15)

    items_bill_live = LabelFrame(items, fg="Black", bg="#badc57")
    items_bill_live.grid(row=1, column=0, padx=25, pady=3)

    Label(manual_mode, text="PRODUCT ID", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0)
    addproduct_e = Entry(manual_mode, width=15, font='arial 15')
    addproduct_e.grid(row=1, column=0, padx=5, pady=8)

    Label(manual_mode, text="QUANTITY", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=1)
    addquantity_e = Entry(manual_mode, width=15, font='arial 15')
    addquantity_e.grid(row=1, column=1, padx=5, pady=8)

    details = LabelFrame(items_bill_live, fg="Black", bg="#badc57", height=510)
    details.grid(row=1, column=0)

    name_detail = LabelFrame(details, fg="Black", bg="#badc57")
    name_detail.grid(row=0, column=0)

    cost_detail = LabelFrame(details, fg="Black", bg="#badc57")
    cost_detail.grid(row=0, column=3)

    mrp_detail = LabelFrame(details, fg="Black", bg="#badc57")
    mrp_detail.grid(row=0, column=1)

    quant_detail = LabelFrame(details, fg="Black", bg="#badc57")
    ...
    Label(name_detail, text="NAME", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac1_e = Text(name_detail, width=18, height=15, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac1_e.grid(row=1, column=0)

    Label(quant_detail, text="QUANTITY", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac2_e = Text(quant_detail, width=13, height=15, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac2_e.grid(row=1, column=0)

    Label(mrp_detail, text="MRP", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac3_e = Text(mrp_detail, width=15, height=15, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac3_e.grid(row=1, column=0)

    Label(cost_detail, text="TOTAL", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac4_e = Text(cost_detail, width=15, height=15, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac4_e.grid(row=1, column=0)

    # bottom window - bill details and action buttons

    F3 = LabelFrame(billing_frame, fg="Black", bg="#badc57")
    F3.place(x=0, y=558, relwidth=1)
    discount = LabelFrame(F3, fg="Black", bg="#badc57")
    discount.grid(row=0, column=0, padx=40, pady=3)

    total_bill = LabelFrame(F3, fg="Black", bg="#badc57")
    total_bill.grid(row=0, column=1, padx=37, pady=3)

    generate_btn = LabelFrame(F3, fg="Black", bg="#badc57")
    generate_btn.grid(row=0, column=4, padx=40, pady=3)

    total_btn = LabelFrame(F3, fg="Black", bg="#badc57")
    total_btn.grid(row=0, column=3, padx=40, pady=3)

    discount_lbl = Label(discount, text="Discount", bg="#badc57", font=('times new roman', 15, 'bold'))
    ...
    total_bill_lbl = Label(total_bill, text="Total Amount", bg="#badc57", font=('times new roman', 15, 'bold'))
    total_bill_lbl.grid(row=0, column=0, padx=10)

    total_bill_txt = Entry(total_bill, width=15, font='arial 15',state='disable')
    ...
    def show_frame():
        ...
        for barcode in decode(img):
            ...
            if myData in barcode_all_list:
                ...
                one_row_qry = "select `pname`, `mrp`, `barcode`, `pid`,`profit`, `quantity` as 'quant_main' from product where `barcode` = %s;" %(myData,)
                ...
                total_profit_new = int(temp_df_pro['profit'].to_string(index=False))
                temp_df_pro['total_profit'] = [total_profit_new]

                master_barcode_list = mstr_df_prd['barcode'].tolist()
                ind_main = mstr_df_prd[mstr_df_prd['barcode'] == myData].index.values
                if myData not in master_barcode_list:
                    q_new_temp = 1
                else:
                    q_new_temp = int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index=False))) + 1
                ...
                if myData in master_barcode_list:
                    if int(float(pid_qry_bill_df['quant_main'].iloc[quant_ind])) - q_new_temp>=0:
                        ...
                        total_new = int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index = False)))*(int(float(mstr_df_prd['mrp'].iloc[ind_main].to_string(index = False))))
                        mstr_df_prd.loc[ind_main,'total'] = [total_new]

                        total_new = int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index=False))) * (int(float(mstr_df_prd['profit'].iloc[ind_main].to_string(index=False))))
                        mstr_df_prd.loc[ind_main, 'total_profit'] = [total_new]
                        sac1_e.configure(state="normal")
                        sac1_e.delete('1.0', END)
                        sac1_e.insert('1.0', mstr_df_prd['pname'].to_string(index=False))
                        sac1_e.configure(state="disable")

                        sac2_e.configure(state="normal")
                        sac2_e.delete('1.0', END)
                        sac2_e.insert('1.0', mstr_df_prd['quantity'].to_string(index=False))
                        sac2_e.configure(state='disable')

                        sac3_e.configure(state="normal")
                        sac3_e.delete('1.0', END)
                        sac3_e.insert('1.0', mstr_df_prd['mrp'].to_string(index=False))
                        sac3_e.configure(state="disable")

                        sac4_e.configure(state="normal")
                        sac4_e.delete('1.0', END)
                        sac4_e.insert('1.0', mstr_df_prd['total'].to_string(index=False))
        ...
        if len(addproduct_e.get())>0 and len(addquantity_e.get())>0:
            if (addproduct_e.get()) in pid_qry_bill_list and int(addquantity_e.get())>0:
                if addproduct_e.get() in (mstr_df_prd['pid'].tolist()):
                    ind_main = mstr_df_prd[mstr_df_prd['pid']==addproduct_e.get()].index.values
                    q_new = int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index = False))) + int(addquantity_e.get())

                    quant_ind = pid_qry_bill_list.index(str(addproduct_e.get()))

                    if int(float(pid_qry_bill_df['quant_main'].iloc[quant_ind])) - q_new >= 0:

                        mstr_df_prd.loc[ind_main, 'quantity'] = q_new
                        total_new = q_new * (int(float(mstr_df_prd['mrp'].iloc[ind_main].to_string(index = False))))
                        mstr_df_prd.loc[ind_main,'total'] = [total_new]
                        total_new = q_new * (int(float(mstr_df_prd['profit'].iloc[ind_main].to_string(index=False))))
                        mstr_df_prd.loc[ind_main, 'total_profit'] = [total_new]
                    ...
                    if int(float(pid_qry_bill_df['quant_main'].iloc[quant_ind])) - int(addquantity_e.get()) >= 0:
                        one_row_qry = "select `pname`, `mrp`, `barcode`, `pid`,`profit`, `quantity` as 'quant_main' from product where `pid` = '%s';" % (addproduct_e.get(),)
                        temp_df_pro = pd.read_sql(one_row_qry, mycon)
                        temp_df_pro['quantity'] = [int(addquantity_e.get())]
                        if int(float(temp_df_pro['quant_main'].to_string(index=False))) > 0:
                            total_new = int(temp_df_pro['mrp'].to_string(index=False)) * int(addquantity_e.get())
                            temp_df_pro['total'] = [total_new]

                            total_new = int(temp_df_pro['profit'].to_string(index=False)) * int(addquantity_e.get())
                        ...
                        messagebox.showinfo("PRODUCT NOT AVAILABLE", "The Quantity of Product is not enough")

            else:
                messagebox.showinfo("PRODUCT NOT FOUND", "The product with pid "+addproduct_e.get()+" not found !")
                addproduct_e.delete(0,END)
                addquantity_e.delete(0,END)
        else:
    ...
    def remove_item_frombill():
        ...
        if len(addquantity_e.get())>0 and len(addproduct_e.get())>0:
            if addproduct_e.get() in mstr_df_prd['pid'].tolist():
                ind_main = mstr_df_prd[mstr_df_prd['pid'] == addproduct_e.get()].index.values
                q_now_ibbill = int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index=False)))
                if q_now_ibbill - int(addquantity_e.get())>=0:
                    ...
                    if int(float(mstr_df_prd['quantity'].iloc[ind_main].to_string(index = False))) == 0:
                        ...
                        mstr_df_prd = mstr_df_prd.drop(mstr_df_prd.index[ind_main])
                    ...
                    messagebox.showinfo("NOT ENOUGH ITEMS", "Not enough items to remove !")
                    addproduct_e.delete(0, END)
                    addquantity_e.delete(0, END)
            else:
                messagebox.showinfo("INVALID INPUT", "No such product found in order list !")
                addproduct_e.delete(0, END)
                addquantity_e.delete(0, END)
        else:
            messagebox.showinfo("INVALID INPUT", "Fill all details first !")
    ...
    def total_amt_bill_gen():
        ...
        if len(discount_txt.get())>0:
            if int(discount_txt.get())<=100:
                ...
                messagebox.showinfo("INVALID INPUT", "Discount must be atmost 100")

        else:
            messagebox.showinfo("INVALID INPUT", "Fill both tax and discount first !")
    ...
    def generate_bill_main():
        ...
        #==========================================================================
            ...
            for asd in range(mstr_df_prd.shape[0]):
                ...
                mrp_main = int(mrpxquant1[asd])*(100 - int(discount_txt.get()))//100
            ...
            for jj in range(num_of_rows):
                qry1 = "INSERT INTO `sales` (`bill_id`, `pname`, `quantity`, `profit_pp`, `barcode`) VALUES ('%s', '%s', '%s', '%s', '%s');" % (bill_id_new,pname_list[jj],quantity_list[jj],proxquant_list[jj],bc_list[jj],)
                cur.execute(qry1)
                mycon.commit()

                if pname_list[jj] in stats_now['pname'].tolist():
                    ind_main = stats_now[stats_now['pname']==pname_list[jj]].index.values
                    sold_new = int(float(stats_now['sold'].iloc[ind_main].to_string(index = False))) + int(quantity_list[jj])
                    net_rev_new = int(float(stats_now['net_revenue'].iloc[ind_main].to_string(index = False))) + int(mrpxquant[jj])
                    net_pro_new = int(float(stats_now['net_profit'].iloc[ind_main].to_string(index = False))) + int(proxquant_list[jj])


                    qry2 = "UPDATE `stats` SET `sold`='%s',`net_revenue`='%s',`net_profit`='%s' WHERE `pname` = '%s';" % (sold_new,net_rev_new,net_pro_new,pname_list[jj],)
                else:
                    qry2 = "INSERT INTO `stats` (`pname`, `sold`, `net_revenue`, `net_profit`) VALUES ('%s', '%s', '%s', '%s')" % (pname_list[jj],quantity_list[jj],mrpxquant[jj],proxquant_list[jj],)

                cur.execute(qry2)
                mycon.commit()

                ind_main = product_now[product_now['pid'] == pid_list_all[jj]].index.values
                quant_new = int(float(product_now['quantity'].iloc[ind_main].to_string(index=False))) - quantity_list[jj]

                sold_new = int(float(product_now['sold'].iloc[ind_main].to_string(index=False))) + quantity_list[jj]

                qry4 = "UPDATE `product` SET `quantity`='%s',`sold`='%s' WHERE `pid` = '%s';" % (quant_new, sold_new, pid_list_all[jj])
                cur.execute(qry4)
                mycon.commit()
            ...
            total_list_bill = pd.DataFrame()
            try:
                billdf_pdf = mstr_df_prd[['pname','mrp','quantity','total'].copy()]
                ...
                billdf_pdf = billdf_pdf.rename({'pname': 'ITEM NAME', 'mrp': 'RATE', 'total': 'VALUE', 'total_all': 'BILL TOTAL'}, axis=1)
                billdf_pdf['VALUE_sub'] = billdf_pdf['VALUE']*(int(discount_txt.get())*0.01)
                ...
                ax.axis('tight')
                ax.axis('off')
                the_table = ax.table(cellText=billdf_pdf.values, colLabels=billdf_pdf.columns, loc='top')
                ...
                msg = EmailMessage()
                msg['Subject'] = str(cname_txt.get()) +' your bill for bill number ' + order_number + ' is attached'
            ...
            except Exception as ex_tmp: print(ex_tmp)
            ...
            messagebox.showinfo("BILL ID "+bill_id_new, "Bill generated successfully !")
            ...
            messagebox.showinfo("CANNOT GENERATE BILL", "Fill all the details/ add some items first !")


    show_frame()

    generate_btn_lbl = Button(generate_btn, text="Generate Bill", width=15, font=('arial', 12, 'bold'),command = generate_bill_main)
    generate_btn_lbl.grid(row=0, column=0, pady=7, padx=5)

    total_btn_lbl = Button(total_btn, text="TOTAL AMOUNT", width=15, font=('arial', 12, 'bold'), command = total_amt_bill_gen)
    total_btn_lbl.grid(row=0, column=0, pady=7, padx=5)

    Button(manual_mode, text="ADD ITEM", width=15, font=('arial', 12, 'bold'),command = add_product_tobill).grid(row=0, column=2, pady=3, padx=5)

    Button(manual_mode, text="REMOVE", width=15, font=('arial', 12, 'bold'),command = remove_item_frombill).grid(row=1, column=2, pady=3, padx=5)
...
def product_frame_funct():
    ...
    F2 = LabelFrame(load_product_frame, fg="Black", bg="#badc57")
    F2.place(x=0, y=0, relwidth=1)

    barcode = LabelFrame(F2, fg="Black", bg="#badc57")
    barcode.grid(row=0, column=1, padx=5, pady=3)

    details = LabelFrame(F2, fg="Black", bg="#badc57", height=510)
    ...
    pid_detail = LabelFrame(details, fg="Black", bg="#badc57")
    pid_detail.grid(row=2, column=0, padx=15, pady=25)

    barcode_detail = LabelFrame(details, fg="Black", bg="#badc57")
    barcode_detail.grid(row=2, column=1, padx=15, pady=25)

    Label(name_detail, text="PRODUCT NAME", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    nd11 = Entry(name_detail, width=15, font='arial 15')
    nd11.grid(row=1, column=0,padx=15, pady=20)

    Label(quant_detail, text="QUANTITY", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    qd11 = Entry(quant_detail, width=15, font='arial 15')
    qd11.grid(row=1, column=0,padx=15, pady=20)

    Label(mrp_detail, text="MRP", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    md11 = Entry(mrp_detail, width=15, font='arial 15')
    md11.grid(row=1, column=0,padx=15, pady=20)

    Label(cost_detail, text="COST", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    cd11 = Entry(cost_detail, width=15, font='arial 15')
    cd11.grid(row=1, column=0,padx=15,pady=20)

    Label(pid_detail, text="PID", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    pid11 = Entry(pid_detail, width=15, font='arial 15')
    pid11.grid(row=1, column=0,padx=15, pady=20)

    Label(barcode_detail, text="BARCODE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0,column=0,pady = 10)
    ...
    def show_frame():
        ...
        for barcode in decode(img):
            ...
            show_all_query111 = "select * from product where `barcode` = '%s';" % (myData,)
    ...
    def update_product_db():
        ...
        if len(nd11.get()) > 0 and len(qd11.get()) > 0 and len(md11.get()) > 0 and len(cd11.get()) > 0 and len(pid11.get()) > 0 and len(bd11.get()) > 0:
            nd, qd, md, cd, pid, bd = str(nd11.get()), int(qd11.get()), int(md11.get()), int(cd11.get()), str(pid11.get()), str(bd11.get())
            date_now = date.today()
            ...
            if pid in pid_db:
                ...
                bd11.configure(state = "disable")
                qd11.delete(0, END)
                messagebox.showinfo("PRODUCT ALREADY IN INVENTORY", "Pls go to Edit/Delete Page to update existing data !")
            ...
            messagebox.showinfo("Alert", "Pls fill all the details before adding to inventory")
    Button(details, text="ADD PRODUCT", width=15, font=('arial', 12, 'bold'), command=update_product_db).grid(row=3,
                                                                                                              column=0,
                                                                                                              pady=14,
                                                                                                              padx=5,
                                                                                                              columnspan=2)
...
def view_product_screen():
    ...
    F1 = LabelFrame(add_product_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)

    fc1 = LabelFrame(F1, fg="Black", bg="#badc57")
    fc1.grid(row=0, column=0, padx=40, pady=3)

    F2 = LabelFrame(F1, fg="Black", bg="#badc57")
    ...
    date_detail = LabelFrame(details, fg="Black", bg="#badc57")
    ...
    Label(name_detail, text="NAME", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac1_e = Text(name_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac1_e.grid(row=1, column=0)

    Label(barcode_detail, text="BARCODE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac2_e = Text(barcode_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac2_e.grid(row=1, column=0)

    Label(mrp_detail, text="MRP", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac3_e = Text(mrp_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac3_e.grid(row=1, column=0)

    Label(cost_detail, text="COST", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac4_e = Text(cost_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac4_e.grid(row=1, column=0)

    Label(quant_detail, text="QUANTITY", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac5_e = Text(quant_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac5_e.grid(row=1, column=0)

    Label(pid_detail, text="PID", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac6_e = Text(pid_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac6_e.grid(row=1, column=0)

    Label(date_detail, text="DATE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac7_e = Text(date_detail, width=16, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    ...
    def view_products_simple():
        ...
        if fpdf.empty:
            ...
            messagebox.showinfo("INVALID PID", "No product with such PID found !")
        else:
            pname11 = fpdf['pname'].to_string(index=False)
            barcode11 = fpdf['barcode'].to_string(index=False)
            mrp11 = fpdf['mrp'].to_string(index=False)
    ...
    manual_mode = LabelFrame(fc1, fg="Black", bg="#badc57")
    ...
    Button(manual_mode, text="SEARCH", width=15, font=('arial', 12, 'bold'), command = view_products_simple).grid(row=0, column=2, pady=3, padx=5)
    Button(manual_mode, text="SAVE AS CSV", width=15, font=('arial', 12, 'bold'), command = save_inv_tocsv).grid(row=1, column=2, pady=3, padx=5)
...
def edit_del_pro():
    ...
    F1 = LabelFrame(del_product_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)
    fc1 = LabelFrame(F1, fg="Black", bg="#badc57")
    ...
    details = LabelFrame(fc2, fg="Black", bg="#badc57", height=510)
    details.grid(row=0, column=0, padx=20, pady=26)
    ...
    Label(name_detail, text="PRODUCT NAME", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,
                                                                                                     pady=10)
    ...
    def upadte_records_db():
        ...
        date_now = date.today()
        qry_main = "UPDATE `product` SET `pname`='%s',`barcode`='%s',`mrp`=%s,`cost`=%s,`quantity`=%s,`profit`=%s,`date_added`='%s',`pid`='%s' WHERE `pid` = '%s';" % (nd111, bd111, md111,cd111,qd111,prt,date_now, pd1011,pid111,)
        cur.execute(qry_main)
        mycon.commit()
    ...
    Button(manual_mode, text="SEARCH", width=15, font=('arial', 12, 'bold'), command=search_product_to_del).grid(row=0, column=1,pady=3,padx=5,rowspan=2)

    Button(details, text="UPDATE", width=15, font=('arial', 12, 'bold'),command = upadte_records_db).grid(row=3,column=0,pady=14,padx=5,columnspan=2)
    Button(details, text="DELETE", width=15, font=('arial', 12, 'bold'),command = del_rec_perma).grid(row=3, column=1, pady=14, padx=5,columnspan=2)
...
def stats_main_all(): #bill all function
    ...
    F1 = LabelFrame(statistics_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)
    ...
    Label(name_detail, text="BILL ID", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac1_e = Text(name_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac1_e.grid(row=1, column=0)

    Label(barcode_detail, text="ITEMS", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac2_e = Text(barcode_detail, width=10, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac2_e.grid(row=1, column=0)

    Label(mrp_detail, text="BILL AMT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac3_e = Text(mrp_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac3_e.grid(row=1, column=0)

    Label(cost_detail, text="BILL PROFIT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac4_e = Text(cost_detail, width=13, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac4_e.grid(row=1, column=0)

    Label(quant_detail, text="DATE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac5_e = Text(quant_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac5_e.grid(row=1, column=0)

    Label(pid_detail, text="CUSTOMER", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac6_e = Text(pid_detail, width=28, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac6_e.grid(row=1, column=0)

    Label(date_detail, text="DISCOUNT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac7_e = Text(date_detail, width=10, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac7_e.grid(row=1, column=0)
    ...
    def view_products_simple():
        global sac1_e, sac2_e, sac3_e, sac4_e, sac5_e, sac6_e, sac7_e
        pid11 = str(pid_view_e.get())
        qry_main = "select * from bill_details where `bill_id` = '%s' ;" % (pid11,)
        main_df_all = pd.read_sql(qry_main,mycon)
        if main_df_all.empty:
            pid_view_e.delete(0, END)
            messagebox.showinfo("INVALID Bill ID", "No Bill with such BILL ID found !")
        else:
            bill_id_all = main_df_all['bill_id'].to_string(index=False)
            items_all = main_df_all['items'].to_string(index=False)
            bill_price_all = main_df_all['bill_price'].to_string(index=False)
    ...
    Label(manual_mode, text="BILL ID", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    pid_view_e = Entry(manual_mode, width=15, font='arial 15')
    pid_view_e.grid(row=1, column=0, padx=5, pady=8)

    Button(manual_mode, text="SEARCH", width=15, font=('arial', 12, 'bold'), command=view_products_simple).grid(row=0,
                                                                                                                column=2,
                                                                                                                pady=3,
                                                                                                                padx=5)
...
def dashboard_main_analysis():
    ...
    F1 = LabelFrame(dashboard_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)

    F2 = LabelFrame(F1, fg="Black", bg="#badc57")
    F2.grid(row =0, column = 0,padx = 30, pady = 25)

    F3 = LabelFrame(F1, fg="Black", bg="#badc57")
    F3.grid(row =1, column = 0, pady = 25,padx = 5)

    rev_month = LabelFrame(F2, fg="Black", bg="#badc57")
    rev_month.grid(row=0, column=0,pady=8, padx=10)

    prof_month = LabelFrame(F2, fg="Black", bg="#badc57")
    prof_month.grid(row=0, column=1,pady=8, padx=10)

    sold_month = LabelFrame(F2, fg="Black", bg="#badc57")
    sold_month.grid(row=0, column=2,pady=8, padx=10)

    top_pro_month = LabelFrame(F2, fg="Black", bg="#badc57")
    top_pro_month.grid(row=0, column=4,pady=8, padx=10)

    rev_today = LabelFrame(F2, fg="Black", bg="#badc57")
    rev_today.grid(row=1, column=0,pady=8, padx=10)

    prof_today = LabelFrame(F2, fg="Black", bg="#badc57")
    prof_today.grid(row=1, column=1,pady=8, padx=10)

    sold_today = LabelFrame(F2, fg="Black", bg="#badc57")
    sold_today.grid(row=1, column=2,pady=8, padx=10)

    stock_avail = LabelFrame(F2, fg="Black", bg="#badc57")
    stock_avail.grid(row=1, column=3,pady=8, padx=10)

    supr_cust = LabelFrame(F2, fg="Black", bg="#badc57")
    supr_cust.grid(row=1, column=4, pady=8, padx=10)

    cust_count = LabelFrame(F2, fg="Black", bg="#badc57")
    cust_count.grid(row=0, column=3, pady=8, padx=10)

    Label(rev_month, text="MONTHLY REVENUE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    rev_month_txt = Entry(rev_month, width=17, font='arial 15', state="disable")
    rev_month_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(prof_month, text="MONTHLY PROFIT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    prof_month_txt = Entry(prof_month, width=17,  font='arial 15', state="disable")
    prof_month_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(sold_month, text="MONTHLY SALES", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    sold_month_txt = Entry(sold_month, width=17, font='arial 15', state="disable")
    sold_month_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(top_pro_month, text="AVERAGE BILL AMT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    top_pro_month_txt = Entry(top_pro_month, width=17, font='arial 15', state="disable")
    top_pro_month_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(rev_today, text="TODAY'S REVENUE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=15)
    rev_today_txt = Entry(rev_today, width=17, font='arial 15', state="disable")
    rev_today_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(prof_today, text="TODAY'S PROFIT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    prof_today_txt = Entry(prof_today, width=17, font='arial 15', state="disable")
    prof_today_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(sold_today, text="TODAY'S SALES", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    sold_today_txt = Entry(sold_today, width=17,font='arial 15', state="disable")
    sold_today_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(stock_avail, text="STOCK", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    stock_avail_txt = Entry(stock_avail, width=17, font='arial 15', state="disable")
    stock_avail_txt.grid(row=1, column=0,pady=8, padx=10)

    Label(cust_count, text="CUSTOMERS", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0, pady=8,padx=10)
    cust_count_txt = Entry(cust_count, width=17, font='arial 15', state="disable")
    cust_count_txt.grid(row=1, column=0, pady=8, padx=10)

    Label(supr_cust, text="PRO CUSTOMERS", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0,pady=8, padx=10)
    supr_cust_txt = Entry(supr_cust, width=17, font='arial 15', state="disable")
    ...
    qry_main = "select SUM(`bill_price`) as 'rev_monthly' from bill_details where MONTH(`bill_date`) = '%s';" % (month_this,)
    ...
    qry_main = "select SUM(`bill_profit`) as 'prof_monthly' from bill_details where MONTH(`bill_date`) = '%s';" % (month_this,)
    df_main = pd.read_sql(qry_main, mycon)
    ...
    qry_main = "select SUM(`items`) as 'items_sold' from bill_details where MONTH(`bill_date`) = '%s';" % (month_this,)
    df_main = pd.read_sql(qry_main, mycon)
    ...
    qry_main = "select SUM(`bill_price`) as 'rev_daily' from bill_details where `bill_date` = '%s';" % (today_date,)
    df_main = pd.read_sql(qry_main,mycon)
    ...
    qry_main = "select SUM(`bill_profit`) as 'prof_daily' from bill_details where `bill_date` = '%s';" % (today_date,)
    df_main = pd.read_sql(qry_main, mycon)
    ...
    qry_main = "select SUM(`items`) as 'items_sold_today' from bill_details where `bill_date` = '%s';" % (today_date,)
    df_main = pd.read_sql(qry_main, mycon)
    ...
    grp1 = LabelFrame(F3, fg="Black", bg="#badc57") # top 5 items
    grp1.grid(row=0, column=0)

    grp2 = LabelFrame(F3, fg="Black", bg="#badc57") #daily sales graph (line)
    grp2.grid(row=0, column=1,padx = 14)

    grp3 = LabelFrame(F3, fg="Black", bg="#badc57") #weekly sale (pie)
...
#====== graph ===========
    ...
    ax2.plot(df_mosa['today_date'].tolist(), df_mosa['today_sale'].tolist(), label = 'revenue')
    ax2.plot(df_mosa['today_date'].tolist(), df_mosa['today_profit'].tolist(), color = 'green', label = 'profit')
...
#===================== graph 3 =========================
    ...
    ax3.bar(x_asdasd-0.2, df_wesa['week_sale'],color='blue',width = 0.4, label='revenue')
    ax3.bar(x_asdasd+0.2, df_wesa['week_profit'],color='green',width = 0.4, label='profit')
...
def sales_analysis_main():
    ...
    F1 = LabelFrame(sales_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)
    ...
    Label(mrp_detail, text="ITEM NAME", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac3_e = Text(mrp_detail, width=23, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac3_e.grid(row=1, column=0)

    Label(cost_detail, text="QUANTITY", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac4_e = Text(cost_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac4_e.grid(row=1, column=0)

    Label(quant_detail, text="PROFIT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac5_e = Text(quant_detail, width=17, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac5_e.grid(row=1, column=0)

    Label(pid_detail, text="BARCODE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac6_e = Text(pid_detail, width=23, height=19, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac6_e.grid(row=1, column=0)
        ...
        for a in range(len(bis_list)):
            ...
            if a != 0:
                if bis_list[a] != bis_list[a - 1]:
                    bis_list_main = bis_list_main[:a] + ['--------------------------'] + bis_list_main[a:]
                    bpa_list = bpa_list[:a] + ['------------------------------------'] + bpa_list[a:]
                    bpra_list = bpra_list[:a] + ['--------------------------'] + bpra_list[a:]
                    bda_list = bda_list[:a] + ['--------------------------'] + bda_list[a:]
                    cna_list = cna_list[:a] + ['------------------------------------'] + cna_list[a:]
...
def bill_display_analysis():
    ...
    F1 = LabelFrame(bill_frame, fg="Black", bg="#badc57")
    F1.place(x=0, y=0, relwidth=1)
    ...
    Label(name_detail, text="PRODUCT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac1_e = Text(name_detail, width=30, height=25, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac1_e.grid(row=1, column=0)

    Label(mrp_detail, text="SOLD", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac3_e = Text(mrp_detail, width=20, height=25, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac3_e.grid(row=1, column=0)

    Label(cost_detail, text="NET REVENUE", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac4_e = Text(cost_detail, width=20, height=25, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac4_e.grid(row=1, column=0)

    Label(quant_detail, text="NET PROFIT", bg="#badc57", font=('times new roman', 15, 'bold')).grid(row=0, column=0)
    sac5_e = Text(quant_detail, width=20, height=25, font='arial 15', yscrollcommand=vsb.set, state="disable")
    sac5_e.grid(row=1, column=0)
